...
namespace CerboGui {
    ...
    private:
        ...
        bool AddButton(std::string name, bool enable, GUITypes::IconData icon, float sizeScaling) {
            ...
            if (ImGui::ImageButton(name.c_str(), (ImTextureID)(intptr_t)icon.texturePtr, ImVec2(IconSize * sizeScaling, IconSize * sizeScaling))) {
                ImGui::EndDisabled();
                return true;
            }
            ImGui::EndDisabled();
        ...
        void CreateAppControls() {
            ...
            if (ImGui::CollapsingHeader("SSH-Daten", ImGuiTreeNodeFlags_DefaultOpen)) {
                ...
                if (AddAppControlButton("Daten lesen", CerboSSH::GetConnectionState() >= SSHTypes::ConnectionState::CONNECTED, readIcon, false)) {
                ...
                for (uint16_t i = 0; i < dataCategories; i++) {
                    ...
                    if (ImGui::TreeNode(buttonName.c_str())) {
                        ...
                        for (uint16_t j = 0; j < countSubPlots; j++) {
                            plotInSubPlotArray[0] = VisualizerInstance->CheckPlotInSubplot(j, i, PDTypes::DragSource::SSHRAW);
                            plotInSubPlotArray[1] = VisualizerInstance->CheckPlotInSubplot(j, i, PDTypes::DragSource::SSHCOMPUTED);
                            plotInSubPlotArray[2] = VisualizerInstance->CheckPlotInSubplot(j, i, PDTypes::DragSource::API);
                            plotInSubPlotArray[3] = VisualizerInstance->CheckPlotInSubplot(j, i, PDTypes::DragSource::MODBUS);
                            ...
                            RemovePlotsFromSubplot(plotInSubPlotArray, plotInSubPlotArrayPrev, j, VisualizerInstance->GetIsInPlotInfo(i).PlotIndex, i);
                        }
        ...
        void RemovePlotsFromSubplot(std::array<bool, PDTypes::noDragSourceTypes> arrayNow, std::array<bool, PDTypes::noDragSourceTypes> arrayLastFrame, uint16_t subPlotIndex, uint16_t plotIndex, uint16_t dataIndex) {
